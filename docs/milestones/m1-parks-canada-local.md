...
|`search_park_availability(query, start_date, end_date, party_size, equipment_type=None, accessible_only=False, nights=None, weekends_only=False)`|Post-M1 addition. Search every campground matching a place name in one call and return a consolidated per-campground summary, so a citizen can ask “anything in Banff?” without the assistant looping (and possibly guessing). Errors are isolated per campground.|
...
|`get_site_details(recreation_area_id, campsite_id, include_photos=False)`                                                                                        |Photos, amenities, accessibility info, description. With `include_photos=True` the photos are fetched and returned as viewable image content (capped at 3), not just URLs, so the assistant can show them.|
