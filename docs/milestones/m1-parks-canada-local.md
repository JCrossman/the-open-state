...
|`create_alert(recreation_area_id, campground_id, start_date, end_date, party_size, equipment_type=None, accessible_only=False, notify_target=None)`              |Persist a watch (SQLite). Poller checks on the camply interval, fires the citizen’s own notification channel. No identity stored; key by opaque id.|
